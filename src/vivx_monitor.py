...
def fetch_lyria_status():
    """Reads the last command sent to Lyria via the Bridge log file."""
    try:
        with open(TEMP_LOG_FILE, 'r') as f:
            lines = f.readlines()
            if lines:
                last_line = lines[-1].strip()
                if "LyriaSteer" in last_line:
                    prompt = last_line.split("LyriaSteer: ")[-1]
                    return f"Steering: {prompt}"
    except FileNotFoundError:
        return "AIX: Lyria Standby (Log Missing)"
    return "AIX: Ready"
...
def display_live_monitor():
    ...
    while True: 
        ...
        # --- AII/AIX (Gemini's Internal State) ---
